...
class UploadProduct extends StatefulWidget {
  ...
  @override
  State<UploadProduct> createState() => _UpdateProfileState();
}

class _UpdateProfileState extends State<UploadProduct> {
  ...
  String? Certificate;
  List Certificate_data = [];
  Future<String> Certificateed() async {

    final prefs = await SharedPreferences.getInstance();
    final userid=prefs.getString("userId");
    final res = await http.get(Uri.parse(
        Apiconst.certificateid+'$userid'));
    final resBody = json.decode(res.body)["data"];
    print("yyyyyyyyyyyyyyyyyy");
    print(resBody);
    setState(() {
      Certificate_data = resBody;
    });
    return "Sucess";
  }
  // the selected value
                    ...
                    child: DropdownButton(
                      hint: Row(
                        children: [
                          ...
                          SizedBox(width: 10,),
                          Text('Quality Certificate ID'),
                        ],
                      ),
                      items: Certificate_data.map((item) {
                        return DropdownMenuItem(
                            child:  Container(
                              ...
                              child: ListTile(
                                ...
                                leading:
                                item['image']==''?Image.asset('assets/noimg.png'):
                                Image.network(Apiconst.imgurl+
                                    item['image'].toString(),
                                  height: 70.r,
                                  width: 70.r,
                                ),
  ...
  Future<void> addoffer(String amount1Controller,
    ...
    final url = Uri.parse(Apiconst.farmer_offer);
    ...
    // Add the form fields
    request.fields['productid'] = '$crop';
    request.fields['quantity'] = "$amount1Controller";
    request.fields['price'] = '$amount2Controller';
    request.fields['pickuplocation'] = '$_draggedAddress';
    request.fields['farmer_id'] = "$userid";
    request.fields['totalamount'] = '$Totalamount';
    request.fields['date'] = '$date';
    request.fields['certficateid'] = '$Certificate';
